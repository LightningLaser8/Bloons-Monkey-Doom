...
let noTextureError = new ImageContainer();
...
function modImage(img, img_x, img_y, img_width, img_height) {
  if (img instanceof ImageContainer) {
    if (!img.image) {
      console.error("Image has not been loaded!");
      img = noTextureError;
    }
    img = img.image;
  }
...
class Vector {
  ...
  getDotProduct(vct) {
    return (
      this.getMagnitude() *
      vct.getMagnitude() *
      Math.cos(this.getAngle() - vct.getAngle())
    );
  }
  interpolate(vct, t) {
    return new Vector(
      this.x + (vct.x - this.x) * t,
      this.y + (vct.y - this.y) * t
    );
  }
  normalise() {
    //In case I need to use this
...
class DrawShape extends Drawer {
  //Draws a simple vector shape.
  #type;
  #colour;
  #outlineColour;
  #outlineWidth;
  ...
  rotDraw(x, y, angle) {
    ...
    stroke(
      this.#outlineColour[0],
      this.#outlineColour[1],
      this.#outlineColour[2],
      this.#outlineColour[3]
    );
    ...
    RADShape(
      ...
      x,
      y,
      this.sizeX * option_scale,
      this.sizeY * option_scale,
      angle
    );
    pop();
  }
...
class DrawImage extends Drawer {
        ...
        if (!this.#errored) {
          console.warn(
            "Error drawing texture " +
              this.image +
              ". Has this texture been preloaded? If so, check spelling and capitalisation."
          );
          this.#errored = true;
        }
      } catch (error) {
...
class DrawMulti {
  //Class for drawing multiple things at once. Does nothing on its own.
  constructor(drawers) {
  ...
  setStart(start) {
    for (let d of this.drawers) {
      if (
        d instanceof DrawLightning ||
        d instanceof DrawMulti ||
        d instanceof DrawLine
      ) {
        d.setStart(start);
      }
    }
...
class DrawAnimated {
  ...
  draw(x, y) {
    this.stepAnimation();
    let slideVector = new Vector(0, this.animation.renderSlide).rotate(
      radians(this.animation.renderRotation)
    );
    this.drawer.rotDraw(
      x + this.animation.renderX + slideVector.x,
      y + this.animation.renderY + slideVector.y,
      radians(this.animation.renderRotation)
    );
  }
  rotDraw(x, y, angle) {
    this.stepAnimation();
    let slideVector = new Vector(0, this.animation.renderSlide).rotate(
      angle + radians(this.animation.renderRotation)
    );
    this.drawer.rotDraw(
      x + this.animation.renderX + slideVector.x,
      y + this.animation.renderY + slideVector.y,
...
class Animation {
  //Template class for animations. Does nothing on its own.
...
class BounceAnimation extends Animation {
  //Simple animation that plays, then plays in reverse.
  #frame;
  #section;
  constructor(changeX, changeY, changeRotation, changeSlide, time) {
...
class LoopAnimation extends Animation {
  //Simple animation that loops continuously.
  #frame;
  #section;
  constructor(changeX, changeY, changeRotation, changeSlide, time) {
    super(changeX, changeY, changeRotation, changeSlide, time);
...
class InfiniteAnimation extends Animation {
  //Simple animation which plays continuously.
  #frame;
  constructor(changeX, changeY, changeRotation, changeSlide, time) {
    super(changeX, changeY, changeRotation, changeSlide, time);
    this.#frame = 0;
...
class DrawOffset {
  //Class for offsetting a drawer. Does nothing on its own.
...
class DrawRotatedOffset {
  ...
  rotDraw(x, y, angle) {
    this.drawer.rotDraw(
      x + this.offsetX,
      y + this.offsetY,
      angle + this.offsetRot
    );
  }
}

class AnimateMulti {
  //Class for playing multiple animations on the same drawer. Does nothing on its own.
...
function drawGlowShapeLight(
  ...
  let drawmulti = [];
  for (let i = 0; i < 4; i++) {
    let newcol = additiveLightenColour(
      [
        outlineColour[0],
        outlineColour[1],
        outlineColour[2],
...
class DrawLine extends Drawer {
  //Pretty boring really. Draws a straight line. That's it.
...
class DrawCurve extends DrawLine {
  //Draws a curve from relative vectors. Doesn't rotate.
  ...
  draw(x, y) {
    ...
    for (let i = 0; i < this.vertices.length; i++) {
      quadraticVertex(
        this.control[i - 1].x + x,
        this.control[i - 1].y + y,
        this.vertices[i].x + x,
        this.vertices[i].y + y
      );
    }
...
class DrawTriangularCurve extends DrawCurve {
  //Draws a line from relative vectors. Doesn't rotate.
  draw(x, y) {
    push();
    stroke(this.colour[0], this.colour[1], this.colour[2], this.colour[3]);
    strokeWeight(this.weight);
...
class DrawLightning extends Drawer {
  //Creates 3 rotated oscillating curves.
  constructor(
    start,
    end,
    colour,
    ...
    if (mode == null || mode == "random") {
      ...
      this.oscScl = [
        oscScl + rnd(-oscVariation, oscVariation),
        oscScl + rnd(-oscVariation, oscVariation),
        oscScl + rnd(-oscVariation, oscVariation),
      ];
      this.iterations = [
        iterations + Math.round(rnd(-iterVariation, iterVariation)),
        iterations + Math.round(rnd(-iterVariation, iterVariation)),
        iterations + Math.round(rnd(-iterVariation, iterVariation)),
      ];
      this.amplitude = [
        amplitude + rnd(-ampVariation, ampVariation),
        amplitude + rnd(-ampVariation, ampVariation),
        amplitude + rnd(-ampVariation, ampVariation),
      ];
    } else if (mode == "fixed") {
  ...
  recalculate() {
    if (this.mode == null || this.mode == "random") {
      ...
      this.oscScl = [
        this.originalOscScl + rnd(-this.oscVariation, this.oscVariation),
        this.originalOscScl + rnd(-this.oscVariation, this.oscVariation),
        this.originalOscScl + rnd(-this.oscVariation, this.oscVariation),
      ];
      this.iterations = [
        this.originalIterations +
          Math.round(rnd(-this.iterVariation, this.iterVariation)),
        this.originalIterations +
          Math.round(rnd(-this.iterVariation, this.iterVariation)),
        this.originalIterations +
          Math.round(rnd(-this.iterVariation, this.iterVariation)),
      ...
      this.amplitude = [
        this.originalAmplitude + rnd(-this.ampVariation, this.ampVariation),
        this.originalAmplitude + rnd(-this.ampVariation, this.ampVariation),
        this.originalAmplitude + rnd(-this.ampVariation, this.ampVariation),
      ];
      ...
      this.oscScl = [
        this.originalOscScl,
        -this.originalOscScl,
        this.originalOscScl * 0.5,
      ];
      this.iterations = [
        Math.round(this.originalIterations * 0.75),
        this.originalIterations,
        this.originalIterations * 2,
      ];
      this.amplitude = [
        -this.originalAmplitude * 0.5,
        this.originalAmplitude,
        this.originalAmplitude * 1.5,
      ];
    }
  }
  drawBetween(start, end) {
  ...
  drawBaseline() {
    generateAnimatedInterpolationCurve(
      this.start,
      this.end,
      frameCount,
      this.amplitude[0],
...
function generateAnimatedInterpolationCurve(
  start,
  end,
  tick,
  amplitude,
  ...
  // function for use in DrawLightning
  let vertices = [],
    control = [];
  ...
  for (let i = 0; i <= iterations; i++) {
    ...
    let smallvct = start
      .subtract(vct)
      .getScaledVector(1 / i)
      .getScaledVector(amplitude / iterations / (dist / 1000));
    let turnedVct = smallvct
      .rotate(radians(60))
      .getScaledVector(Math.cos(radians(tick * oscScl)));
    if (i % 2 == 0) {
...
function blendColours(col1, col2, col1Factor) {
  ...
  let newCol1 = [
    ...
    col1[1] * col1Factor,
    col1[2] * col1Factor,
    (col1[3] ?? 255) * col1Factor,
  ];
  let newCol2 = [
    col2[0] * col2Factor,
    col2[1] * col2Factor,
    col2[2] * col2Factor,
    (col2[3] ?? 255) * col2Factor,
  ];
  let newCol = [
    newCol1[0] + newCol2[0],
    newCol1[1] + newCol2[1],
